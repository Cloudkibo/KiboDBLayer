...
*/
const LogicLayer = require('./automation_queue.logiclayer')
...
exports.createOneAutomaticQueueObject = (body) => {
...
exports.updateAutomationQueue = (body) => {
  if (body.purpose) {
...
exports.findAutomationQueueUsingQuery = (body) => {
  if (body.purpose) {
    // If purpose found, then proceed
    if (body.purpose === 'aggregate') {
      let aggregateQuery = LogicLayer.prepareMongoAggregateQuery(body)
      // If not validated
      logger.serverLog('Aggregate:', util.inspect(aggregateQuery))
...
exports.deleteAutomationQueue = (body) => {
  if (body.purpose) {
    let query = body.match
    // If purpose found, then proceed
