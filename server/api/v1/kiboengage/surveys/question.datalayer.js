...
const MongoInterface = require('./question_interface_mongo')
const logger = require('../../../../components/logger')
const TAG = '/api/v1/kiboengage/surveys/question.datalayer.js'

const util = require('util')

exports.findAllQuestionObjects = () => {
  return MongoInterface.find()
}

exports.createOneQuestionObject = (body) => {
  if (LogicLayer.validateQuestionPayload(body)) return MongoInterface.create(body)
  else return new Promise((resolve, reject) => { reject(new Error('Payload is not valid')) })
}

exports.updateQuestion = (body) => {
  if (body.purpose) {
...
exports.findQuestionUsingQuery = (body) => {
  if (body.purpose) {
    // If purpose found, then proceed
