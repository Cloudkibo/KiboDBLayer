...
*/
const LogicLayer = require('./sequence.logiclayer')
const MongoInterface = require('./message_interface_mongo')
const logger = require('../../../../components/logger')
const TAG = '/api/v1/kiboengage/sequence/message.datalayer.js'

const util = require('util')

exports.findAllMessageObjects = () => {
  return MongoInterface.find()
}

exports.createOneMessageObject = (body) => {
  if (LogicLayer.validateMessagePayload(body)) return MongoInterface.create(body)
  else return new Promise((resolve, reject) => { reject(new Error('Payload is not valid')) })
}

exports.updateMessage = (body) => {
  if (body.purpose) {
...
exports.findMessageUsingQuery = (body) => {
  if (body.purpose) {
    // If purpose found, then proceed
